...
void handleRequestAccess(AsyncWebServerRequest* request, uint8_t* data, size_t len, size_t index, size_t total) {
  ...
  Serial.println("File Requested: " + requestedFile);

  if (LittleFS.exists("/" + requestedFile)) {
    accessWaiting = true;
    accessGranted = false;
    accessRequestStartTime = millis();
    Serial.println("Access Waiting for UID...");
...
void handleGetRequestedFile(AsyncWebServerRequest* request) {
  Serial.println("Requested File: " + requestedFile);
  if (accessWaiting || accessGranted) {
    request->send(200, "text/plain", requestedFile);
  } else {
...
void handleGetCountdown(AsyncWebServerRequest* request) {
  unsigned long currentTime = millis();
  int remainingTime = 0;

  if (accessWaiting) {
    remainingTime = max(0, (int)((ACCESS_TIMEOUT - (currentTime - accessRequestStartTime)) / 1000));
  } else if (accessGranted) {
    remainingTime = max(0, (int)((ACCESS_TIMEOUT - (currentTime - accessGrantedStartTime)) / 1000));
  }
  Serial.println("Countdown Sent: " + String(remainingTime));
  request->send(200, "text/plain", String(remainingTime));
}
...
void setup() {
  ...
  server.on("/getRequestedFile", HTTP_GET, handleGetRequestedFile);
  server.on("/getCountdown", HTTP_GET, handleGetCountdown);
